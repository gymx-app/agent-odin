...
import { createReadinessCheckHandler } from '../_bundles/readiness-check-handler.js';

let handler: ReturnType<typeof createReadinessCheckHandler> | null = null;

const getHandler = () => {
  if (!handler) {
    handler = createReadinessCheckHandler();
  }
  return handler;
};
